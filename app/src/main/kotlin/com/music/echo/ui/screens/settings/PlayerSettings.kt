...
import echo.music.***REMOVED***.constants.AutomixDebugOverlayKey
...
import echo.music.***REMOVED***.constants.EnableGoogleCastKey
...
import echo.music.***REMOVED***.constants.SeekExtraSeconds
...
import echo.music.***REMOVED***.constants.EnableExportAsMp3Key
...
highlightKey: String? = null) {
    ...
    val (dataSaverEnabled, onDataSaverEnabledChange) = rememberPreference(
        key = echo.music.***REMOVED***.constants.DataSaverEnabledKey,
        defaultValue = false
    )
    ...
    val (playbackEngine, onPlaybackEngineChange) = rememberEnumPreference(
        echo.music.***REMOVED***.constants.PlaybackEngineKey,
        defaultValue = echo.music.***REMOVED***.constants.PlaybackEngine.AUTO
    )

    val (downloadQuality, onDownloadQualityChange) = rememberEnumPreference(
        echo.music.***REMOVED***.constants.DownloadQualityKey,
        defaultValue = echo.music.***REMOVED***.constants.DownloadQuality.YOUTUBE
    )
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(echo.music.***REMOVED***.constants.DownloadQuality.YOUTUBE),
            valueText = {
                when (it) {
                    echo.music.***REMOVED***.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    else -> ""
                }
            }
        )
    ...
    if (showPlaybackEngineDialog) {
        EnumDialog(
            ...
            onSelect = {
                ...
                echo.music.***REMOVED***.utils.YTPlayerUtils.playbackEngine = it
                showPlaybackEngineDialog = false
            },
            title = "Playback Engine",
            current = playbackEngine,
            values = listOf(
                echo.music.***REMOVED***.constants.PlaybackEngine.POTOKEN,
                echo.music.***REMOVED***.constants.PlaybackEngine.BRAVEPIPE,
                echo.music.***REMOVED***.constants.PlaybackEngine.AUTO,
            ),
            valueText = {
                when (it) {
                    echo.music.***REMOVED***.constants.PlaybackEngine.POTOKEN -> "PoToken (Recommended)"
                    echo.music.***REMOVED***.constants.PlaybackEngine.BRAVEPIPE -> "BravePipe (NewPipe)"
                    echo.music.***REMOVED***.constants.PlaybackEngine.AUTO -> "Auto (Try Both)"
                }
            },
            valueDescription = {
                when (it) {
                    echo.music.***REMOVED***.constants.PlaybackEngine.POTOKEN -> "Uses WebView PoToken + CipherDeobfuscator. Most reliable and future-proof."
                    echo.music.***REMOVED***.constants.PlaybackEngine.BRAVEPIPE -> "Uses NewPipe extractor for stream resolution. Lightweight but may break with YouTube updates."
                    echo.music.***REMOVED***.constants.PlaybackEngine.AUTO -> "Tries PoToken first, falls back to BravePipe if it fails."
                }
            }
        )
    }
