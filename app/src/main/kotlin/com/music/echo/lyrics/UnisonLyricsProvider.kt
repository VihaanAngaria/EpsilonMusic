...
import echo.music.***REMOVED***.constants.UnisonLyricsEnabledKey
...
object UnisonLyricsProvider : LyricsProvider {
    ...
    private fun convertIfTTML(content: String): String {
        return if (content.trimStart().startsWith("<tt", ignoreCase = true)) {
            val parsedLines = echo.music.***REMOVED***.betterlyrics.TTMLParser.parseTTML(content)
            echo.music.***REMOVED***.betterlyrics.TTMLParser.toLRC(parsedLines)
        } else {
            content
        }
    }
