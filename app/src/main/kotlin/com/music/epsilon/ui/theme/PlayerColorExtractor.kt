...
package epsilon.music.***REMOVED***.ui.theme
...
object PlayerColorExtractor {
    ...
    suspend fun extractGradientColors(
        ...
        val primaryColor = if (bestSwatch != null) {
            ...
            if (isColorVibrant(bestColor)) {
                enhanceColorEpsilonness(bestColor, 1.3f)
            } else {
                
                enhanceColorEpsilonness(fallbackDominant, 1.1f)
            }
        } else {
            enhanceColorEpsilonness(fallbackDominant, 1.1f)
        }
    ...
    private fun enhanceColorEpsilonness(color: Color, saturationFactor: Float = 1.4f): Color {
        val argb = color.toArgb()
        val hsv = FloatArray(3)
        android.graphics.Color.colorToHSV(argb, hsv)
