...
import com.music.epsilon.supabase.model.RecentlyPlayedDto
...
class HistorySyncRepository @Inject constructor(
    ...
    suspend fun listListeningHistory(limit: Int = 200): List<com.music.epsilon.supabase.model.ListeningHistoryDto> = try {
        postgrest.from("listening_history").select {
            order("played_at", Order.DESCENDING)
            range(0, (limit - 1).coerceAtLeast(0))
        }.decodeList<com.music.epsilon.supabase.model.ListeningHistoryDto>()
    } catch (e: Exception) {
