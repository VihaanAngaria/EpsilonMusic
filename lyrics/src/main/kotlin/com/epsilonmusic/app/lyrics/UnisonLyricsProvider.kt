...
import com.epsilonmusic.app.unison.Unison
import com.epsilonmusic.app.constants.UnisonLyricsEnabledKey
...
object UnisonLyricsProvider : LyricsProvider {
    ...
    private fun convertIfTTML(content: String): String {
        return if (content.trimStart().startsWith("<tt", ignoreCase = true)) {
            val parsedLines = com.epsilonmusic.app.betterlyrics.TTMLParser.parseTTML(content)
            com.epsilonmusic.app.betterlyrics.TTMLParser.toLRC(parsedLines)
        } else {
            content
        }
    }
